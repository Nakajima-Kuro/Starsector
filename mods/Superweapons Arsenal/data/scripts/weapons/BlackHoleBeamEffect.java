        ...
        if (beam.getWeapon().getChargeLevel() == 1f){
			...
			if (!Implode){
				...
				DamagingExplosionSpec Impact = new DamagingExplosionSpec(
					...
					3000f,				//maxDamage
					500,		        //minDamage
					CollisionClass.HITS_SHIPS_ONLY_FF,			//collisionClass
					CollisionClass.ASTEROID,					//collisionClassByFighter
					30f,				//particleSizeMin,
					50f,				//particleSizeRange
					1f,					//particleDuration
					200, 				//particleCount
					EXPLOSION_COLOR,
					EXPLOSION_COLOR);
				...
				Impact.setDamageType(DamageType.FRAGMENTATION);
				engine.spawnDamagingExplosion(Impact, beam.getSource(), point, true);
				...
				SW_BlackHolePlugin.addSingularity(new Vector2f(point), power * 0.56f, neighbours, lightCrafts, beam.getSource());
