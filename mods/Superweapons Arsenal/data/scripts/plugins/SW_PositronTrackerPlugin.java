        ...
        for (ShipAPI Ship : Global.getCombatEngine().getShips()){
			if (Ship.isAlive()){
				...
				if (Ship.getMutableStats().getDynamic().getStat("SW_POSITRON_SHIELD").getModifiedValue() > 0f){
					Ship.getMutableStats().getDynamic().getStat("SW_POSITRON_SHIELD").modifyFlat("SW_PS2", Ship.getMutableStats().getDynamic().getStat("SW_POSITRON_SHIELD").getModifiedValue() + amount/2f);
				...
				if (Ship.getMutableStats().getDynamic().getStat("SW_POSITRON").getModifiedValue() > 0f){
					Ship.getMutableStats().getDynamic().getStat("SW_POSITRON").modifyFlat("SW_P2", Ship.getMutableStats().getDynamic().getStat("SW_POSITRON").getModifiedValue() + amount/2f);
