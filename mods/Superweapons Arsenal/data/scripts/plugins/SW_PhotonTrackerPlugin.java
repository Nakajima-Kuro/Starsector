        ...
        for (ShipAPI Ship : Global.getCombatEngine().getShips()){
			if (Ship.isAlive()){
				...
				if (Ship.getMutableStats().getDynamic().getStat("SW_PHOTON_SHIELD").getModifiedValue() > 0f){
					Ship.getMutableStats().getDynamic().getStat("SW_PHOTON_SHIELD").modifyFlat("SW_PS2", Ship.getMutableStats().getDynamic().getStat("SW_PHOTON_SHIELD").getModifiedValue() + amount/2f);
				...
				if (Ship.getMutableStats().getDynamic().getStat("SW_PHOTON").getModifiedValue() > 0f){
					Ship.getMutableStats().getDynamic().getStat("SW_PHOTON").modifyFlat("SW_P2", Ship.getMutableStats().getDynamic().getStat("SW_PHOTON").getModifiedValue() + amount/2f);
