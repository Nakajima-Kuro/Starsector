...
public class SWP_PlasmaFlameEffect implements EveryFrameWeaponEffectPlugin, OnHitEffectPlugin, OnFireEffectPlugin {
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        if (!shields && target instanceof ShipAPI) {
            ShipAPI ship = (ShipAPI) target;
            if (SWPModPlugin.templarsExists && ship.getVariant().getHullMods().contains("tem_latticeshield") && TEM_LatticeShield.shieldLevel(ship) > 0f) {
                shields = true;
            }
        }
        if (!shields) {
